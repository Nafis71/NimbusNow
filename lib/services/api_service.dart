...
class ApiService{
  Future<Map<String, dynamic>> fetchWeatherData(String location) async {
    Response response =  await http.get(Uri.parse("${currentWeatherApiBaseUrl}key=$apiKey&q=$location&aqi=no  "));
  ...
  Future<List<dynamic>> fetchForecastData(String location) async {
    Response response = await http.get(Uri.parse("${forecastApiBaseUrl}key=$apiKey&q=$location&days=1&aqi=no&alerts=no"));
    if(response.statusCode == 200){
      Map<String,dynamic> jsonData = jsonDecode(response.body);
